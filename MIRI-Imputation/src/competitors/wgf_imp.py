...
class NeuralGradFlowImputer(object):
    ...
    def fit_transform(self, X, verbose=True, report_interval=10, X_true=None, OTLIM=5000, clamp=False):
        ...
        if d == 3 * 32 * 32 or d == 3 * 64 * 64:
            ...
            self.mlp_model = CNNNet2(d).to(device=self.device, dtype=torch.float64)
        else:
            self.mlp_model = ToyMLP(input_dim=d, units=self.mlp_hidden).to(device=self.device, dtype=torch.float64)
        
        self.score_net = Energy(net=self.mlp_model).to(device=self.device, dtype=torch.float64)

...
def rectified_impute(X0, M, Xstar, modelclass, max_rounds = 100, clamp = False, callback = None, verbose=False, 
    ...
    # save previous imputation to return it if nan is encountered
    Xprev = X0.clone()
    ...
    try:
        for round in range(max_rounds):
            if torch.isnan(X0).any():
                print("Imputation contains NaN values. Returning the previous result.")
                X0 = Xprev.clone()
                break
            
            Xprev = X0.clone()
            
            model = modelclass(d).to(device=device, dtype=X0.dtype).double()
            ...
            if torch.isnan(mmd_round):
                print("MMD yields NaN. Returning the current result.")
                break
            ...
            for epoch in tqdm(range(maxepochs)):
                for iter, (Z0i, X1i) in enumerate(trainloader):
                    X1i = X1i.to(device=device, dtype=X0.dtype)
                    X0i = Z0i[:, :d].to(device=device, dtype=X0.dtype)
                    Mi = Z0i[:, d:].to(device=device, dtype=X0.dtype)
            ...
            for i in tqdm(range(0, n, imputebatch)):
                Z0i = Z0[i:i+imputebatch, :].clone().to(device=device, dtype=X0.dtype)

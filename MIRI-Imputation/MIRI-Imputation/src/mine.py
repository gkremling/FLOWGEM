...
def MINE(x0,y0):
    ...
    model = MLP(x.shape[1], y.shape[1]).to(device=device, dtype=x0.dtype).double()
    ...
    for epoch in tqdm(range(100)):
        for iter, (xi, yi) in enumerate(trainloader):
            xi = xi.to(device=device, dtype=x0.dtype)
            yi = yi.to(device=device, dtype=x0.dtype)
    ...
    return -loss(xtest.to(device=device, dtype=x0.dtype), ytest.to(device=device, dtype=x0.dtype), model).item(), model

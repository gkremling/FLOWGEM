...
class MissDiffImage:
    ...
    def __init__(
        ...
        if inputdim == 32 * 32 * 3 or inputdim == 3 * 64 * 64:
            ...
            self.network = CNNNet2_T(inputdim).to(device=device, dtype=torch.float64)
        else:
            from src.mlp import MLP2
            self.network = MLP2(inputdim).to(device=device, dtype=torch.float64)
